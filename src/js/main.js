...
const activeToggle = function(clickEvent){
    const toggleContactsEvent = new CustomEvent('toggleContactsEvent', {
        detail: {
            buttonClicked: clickEvent.target.closest('.container').querySelector('.contacts'),
        }
    });

    allContacts.forEach(contact => contact.dispatchEvent(toggleContactsEvent));
}

const toggleContactsVisibility = function(toggleEvent) {
...
allContacts.forEach(contact => contact.addEventListener('toggleContactsEvent', toggleContactsVisibility));
buttons.forEach(button => button.addEventListener('click', activeToggle));

// //DELEGATION EVENT
// const buttons = Array.from(document.querySelectorAll('.banner__btn'));

// const toggleContacts = new Event = () => {
//     console.log(document.activeElement);
// }

// document.addEventListener('toggleContactsEvent');

// buttons.forEach(button => button.addEventListener('click', toggleContacts));

// // o click e no body ou no main
